...
import { resolveStreamPhase, getPhaseConfidence } from './stream-phase-detector.js';
...
const SESSION_MARKER_REGEX = /(Launched new game session|Resumed existing game session|Connection terminated|stage .* failed|Configuring with format|Using codec|Average end-to-end client latency|Average hardware decoder latency)/i;
...
export async function parseAppFocusLog(logcatPath, yearHint = new Date().getFullYear(), {
  ...
  const addMetric = (ts, type, value, unit, line, phase, confidence, metricSource = 'legacy_pattern') => {
    if (!Number.isFinite(value)) return;
    result.metricSamples.push({ ts, type, value, unit, line, phase, confidence, metricSource });
    incrementCounter(result.metricSourceCounts, metricSource);
  };

  const addAnomaly = (ts, type, line, phase, confidence) => {
    const key = `${ts.getTime()}|${type}|${line}`;
    if (anomalyDedup.has(key)) return;
    anomalyDedup.add(key);
    result.anomalyEvents.push({ ts, type, line, phase, confidence });
  };
  ...
  for await (const line of rl) {
    ...
    const phase = resolveStreamPhase(ts, streamDetection);
    ...
    if (internalStats) {
      addMetric(ts, 'fps_total', internalStats.fpsTotal, 'fps', line, phase, confidence, 'internal_stats');
      addMetric(ts, 'fps_rx', internalStats.fpsRx, 'fps', line, phase, confidence, 'internal_stats');
      addMetric(ts, 'fps_rd', internalStats.fpsRd, 'fps', line, phase, confidence, 'internal_stats');
      addMetric(ts, 'loss_frames', internalStats.lossFrames, 'count', line, phase, confidence, 'internal_stats');
      addMetric(ts, 'loss_total', internalStats.lossTotal, 'count', line, phase, confidence, 'internal_stats');
      addMetric(ts, 'loss_pct', internalStats.lossPct, '%', line, phase, confidence, 'internal_stats');
      addMetric(ts, 'loss_events', internalStats.lossEvents, 'count', line, phase, confidence, 'internal_stats');
      addMetric(ts, 'rtt_ms', internalStats.rttMs, 'ms', line, phase, confidence, 'internal_stats');
      addMetric(ts, 'rtt_var_ms', internalStats.rttVarMs, 'ms', line, phase, confidence, 'internal_stats');
      addMetric(ts, 'decode_ms', internalStats.decodeMs, 'ms', line, phase, confidence, 'internal_stats');
      addMetric(ts, 'render_ms', internalStats.renderMs, 'ms', line, phase, confidence, 'internal_stats');
      addMetric(ts, 'total_ms', internalStats.totalMs, 'ms', line, phase, confidence, 'internal_stats');
      addMetric(ts, 'host_latency_min_ms', internalStats.hostLatencyMinMs, 'ms', line, phase, confidence, 'internal_stats');
      addMetric(ts, 'host_latency_max_ms', internalStats.hostLatencyMaxMs, 'ms', line, phase, confidence, 'internal_stats');
      addMetric(ts, 'host_latency_avg_ms', internalStats.hostLatencyAvgMs, 'ms', line, phase, confidence, 'internal_stats');
      result.internalStatsSamples.push({ ts, ...internalStats, phase, confidence, line });
      hasMetricFromLine = true;
    }

    const fpsPair = message.match(/Rx\s+(\d+(?:\.\d+)?)\s*\/\s*Rd\s+(\d+(?:\.\d+)?)\s*FPS/i);
    if (fpsPair) {
      addMetric(ts, 'fps_rx', toNumber(fpsPair[1]), 'fps', line, phase, confidence, 'legacy_pattern');
      addMetric(ts, 'fps_rd', toNumber(fpsPair[2]), 'fps', line, phase, confidence, 'legacy_pattern');
      hasMetricFromLine = true;
    }

    const rttJitter = message.match(/(\d+(?:\.\d+)?)\s*±\s*(\d+(?:\.\d+)?)\s*ms/i);
    if (rttJitter) {
      addMetric(ts, 'rtt_ms', toNumber(rttJitter[1]), 'ms', line, phase, confidence, 'legacy_pattern');
      addMetric(ts, 'jitter_ms', toNumber(rttJitter[2]), 'ms', line, phase, confidence, 'legacy_pattern');
      hasMetricFromLine = true;
    }

    const lossRate = message.match(/(?:丢帧率|跳帧率|loss\s*rate)\s*[:：]?\s*(-?\d+(?:\.\d+)?)\s*%/i);
    if (lossRate) {
      addMetric(ts, 'lost_frame_pct', toNumber(lossRate[1]), '%', line, phase, confidence, 'legacy_pattern');
      hasMetricFromLine = true;
    }

    const preciseSync = message.match(/精确同步:\s*(\d+)帧,\s*跳帧:\s*(\d+),\s*平均误差:\s*(-?\d+(?:\.\d+)?)ms/);
    if (preciseSync) {
      addMetric(ts, 'precise_sync_frames', toNumber(preciseSync[1]), 'count', line, phase, confidence, 'legacy_pattern');
      addMetric(ts, 'precise_sync_skipped_frames', toNumber(preciseSync[2]), 'count', line, phase, confidence, 'legacy_pattern');
      addMetric(ts, 'precise_sync_avg_error_ms', toNumber(preciseSync[3]), 'ms', line, phase, confidence, 'legacy_pattern');
      hasMetricFromLine = true;
    }

    const preciseSyncRate = message.match(/\[精确同步:\s*(\d+)渲染\/(\d+)接收,\s*跳帧率:\s*(-?\d+(?:\.\d+)?)%\]/);
    if (preciseSyncRate) {
      addMetric(ts, 'precise_sync_rendered_frames', toNumber(preciseSyncRate[1]), 'count', line, phase, confidence, 'legacy_pattern');
      addMetric(ts, 'precise_sync_received_frames', toNumber(preciseSyncRate[2]), 'count', line, phase, confidence, 'legacy_pattern');
      addMetric(ts, 'precise_sync_skip_rate_pct', toNumber(preciseSyncRate[3]), '%', line, phase, confidence, 'legacy_pattern');
      hasMetricFromLine = true;
    }

    const pendingAudio = message.match(/Too much pending audio data:\s*(-?\d+(?:\.\d+)?)\s*ms/i);
    if (pendingAudio) {
      addMetric(ts, 'pending_audio_ms', toNumber(pendingAudio[1]), 'ms', line, phase, confidence, 'legacy_pattern');
      hasMetricFromLine = true;
    }

    const timeoutConfig = message.match(/TimeoutConfig\{connect=(\d+)ms,\s*read=(\d+)ms,\s*stun=(\d+)ms\}/i);
    if (timeoutConfig) {
      addMetric(ts, 'timeout_connect_ms', toNumber(timeoutConfig[1]), 'ms', line, phase, confidence, 'legacy_pattern');
      addMetric(ts, 'timeout_read_ms', toNumber(timeoutConfig[2]), 'ms', line, phase, confidence, 'legacy_pattern');
      addMetric(ts, 'timeout_stun_ms', toNumber(timeoutConfig[3]), 'ms', line, phase, confidence, 'legacy_pattern');
      hasMetricFromLine = true;
    }

    const connStats = message.match(/Stats\{success=(\d+),\s*failure=(\d+),\s*rate=([\d.]+)%.*avg_response=(\d+)ms\}/i);
    if (connStats) {
      addMetric(ts, 'conn_success_count', toNumber(connStats[1]), 'count', line, phase, confidence, 'legacy_pattern');
      addMetric(ts, 'conn_failure_count', toNumber(connStats[2]), 'count', line, phase, confidence, 'legacy_pattern');
      addMetric(ts, 'conn_success_rate_pct', toNumber(connStats[3]), '%', line, phase, confidence, 'legacy_pattern');
      addMetric(ts, 'conn_avg_response_ms', toNumber(connStats[4]), 'ms', line, phase, confidence, 'legacy_pattern');
      hasMetricFromLine = true;
    }

    const e2eLatency = message.match(/Average end-to-end client latency:\s*(\d+)\s*ms/i);
    if (e2eLatency) {
      addMetric(ts, 'e2e_latency_ms', toNumber(e2eLatency[1]), 'ms', line, phase, confidence, 'legacy_pattern');
      hasMetricFromLine = true;
    }

    const decoderLatency = message.match(/Average hardware decoder latency:\s*(\d+)\s*ms/i);
    if (decoderLatency) {
      addMetric(ts, 'decoder_latency_ms', toNumber(decoderLatency[1]), 'ms', line, phase, confidence, 'legacy_pattern');
      hasMetricFromLine = true;
    }

    const displayRefresh = message.match(/Display refresh rate:\s*(-?\d+(?:\.\d+)?)\s*Hz/i);
    if (displayRefresh) {
      addMetric(ts, 'display_refresh_hz', toNumber(displayRefresh[1]), 'hz', line, phase, confidence, 'legacy_pattern');
      hasMetricFromLine = true;
    }
    ...
    for (const p of anomalyPatterns) {
      ...
      addAnomaly(ts, p.type, line, phase, confidence);
    }

    if (priority && /[WEFA]/.test(priority) && (limeLogTag || hitAnomalyPattern || sessionMarker)) {
      addAnomaly(ts, 'warn_or_error', line, phase, confidence);
    }
